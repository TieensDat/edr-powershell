...
def main():
    parser = argparse.ArgumentParser(description="Evaluate collected PowerShell Gallery scripts with PythonAgent detection logic.")
    parser.add_argument("--files-manifest", required=True)
    parser.add_argument("--agent-path", default="PythonAgent/PythonAgent.py")
    parser.add_argument("--output-dir", default="datasets/powershell_gallery/pilot_100/reports")
    parser.add_argument("--max-files", type=int, default=0, help="0 means evaluate all files.")
    parser.add_argument("--max-files-per-package", type=int, default=0, help="0 means no per-package cap.")
    ...
    if args.max_files_per_package and args.max_files_per_package > 0:
        capped_rows = []
        per_package_counts = Counter()
        for row in rows:
            package_name = row.get("package_name", "")
            if per_package_counts[package_name] >= args.max_files_per_package:
                continue
            capped_rows.append(row)
            per_package_counts[package_name] += 1
        rows = capped_rows
    ...
    for index, row in enumerate(rows, start=1):
        ...
        event = {
            "source": "powershell_gallery_static_eval",
            "pid": 0,
            "ppid": 0,
